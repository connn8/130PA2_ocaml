...
let rec build (rand,depth) = 
   let x = rand(0,7) in
   match depth with
   |0 -> if x < 3 then buildX() else buildY()
   |_ -> match x with 
         |0 when depth < 3 -> buildX()     (*allows for x and y to be part of expression*)
         |1 when depth < 3 -> buildY()     (*without cutting it too short*)
...
let rec build2 (rand,depth) = 
   let x = rand(0,9) in
   match depth with
   |0 -> if x < 4 then buildX() else buildY()
   |_ -> match x with 
         |0 when depth < 3 -> buildX()     (*allows for x and y to be part of expression*)
         |1 when depth < 3 -> buildY()     (*without cutting it too short*)
         |2 -> buildCosine(build(rand,depth-1))
	 ...
	 |4 -> buildAverage(build(rand,depth-1),build(rand,depth-1))
         |5 -> buildTimes(build(rand,depth-1),build(rand,depth-1))
         |6 -> buildTriple(build(rand,depth-1),build(rand,depth-1),build(rand,depth-1))
         |7 -> buildMean(build(rand,depth-1),build(rand,depth-1),build(rand,depth-1))
         |_ -> buildSine(build(rand,depth-1));;
